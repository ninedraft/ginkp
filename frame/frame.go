...
func (frame *Frame) writeCommandWithArgs(command command.CommandCode, args []byte) {
	fBytes := frame.Bytes()
	frame.prepare()
...
func (frame *Frame) writeCommand(command CommandCode) {
	fBytes := frame.Bytes()
	frame.prepare()
	fBytes[2] = byte(command)
	frame.UpdateLen()
...
func (frame *Frame) M_NUL(msg string) {
	frame.writeCommandWithArgs(M_NUL, []byte(msg))
}
...
func (frame *Frame) M_PWD(password string) {
	frame.writeCommandWithArgs(M_PWD, []byte(password))
}
...
func (frame *Frame) M_FILE(file *FileConfig) {
	frame.writeCommandWithArgs(M_FILE, []byte(file.String()))
}
...
func (frame *Frame) M_GOT(file *FileConfig) {
	frame.writeCommandWithArgs(M_FILE, []byte(file.StringWithoutOff()))
}
...
func (frame *Frame) M_ERR(err error) {
	frame.writeCommandWithArgs(M_ERR, []byte(err.Error()))
}
...
func (frame *Frame) CommandCode() (CommandCode, error) {
	...
	if fLen >= 3 {
		...
		return CommandCode(fBytes[2]), nil
	}
...
func (frame *Frame) WriteDataTo(w io.Writer) (int64, error) {
	...
	buf := bytes.NewBuffer(frame.Bytes()[3:])
	n, err := buf.WriteTo(w)
	frame.UpdateLen()
	return n, err
}
